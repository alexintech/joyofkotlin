    ...
    init {
        ...
        "buyDonut" {
            forAll(Gen.list(PurchaseGenerator())) { purchases: List<Purchase> ->
                ...
                val paymentMap: Map<CreditCard, Int> = groupedPaymentList.fold(mapOf()) {
                    map, (creditCard, amount) -> map + (Pair(creditCard, amount))
                }
                val creditCardSet: Set<CreditCard> = purchases.fold(setOf()) {
                        set, purchase -> set + purchase.payment.creditCard
                }
                val charges: List<Pair<CreditCard, List<Payment>>> = creditCardSet.map {
                    creditCard -> Pair(creditCard, paymentList.filter { it.creditCard == creditCard })
                }
                charges.fold(true) {
                        acc, p -> acc && p.second.sumBy { it.amount } == paymentMap[p.first]
                }
            }
        }
    }
...
class PurchaseGenerator : Gen<Purchase> {
    ...
    override fun random(): Sequence<Purchase> {
        ...
        return Gen.choose(0, 100).map {
            if (random.nextBoolean()) buyDonuts(it, creditCard1) else buyDonuts(it, creditCard2)
        }.random()
    }
}